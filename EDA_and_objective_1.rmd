...
#graphic of missing information
mis <- df %>% filter(is.na(workclass))
mis2 <-df %>% filter(is.na(occupation))
mis3 <- df %>% filter(is.na(native.country))
allmis <- rbind(mis,mis2,mis3)
df %>% ggplot(aes(x=income, fill = income)) + geom_bar(title = )
allmis %>% ggplot(aes(x=income, fill = income)) + geom_bar()
df2 %>% ggplot(aes(x=income, fill = income)) + geom_bar()
...
library(regclass)
df4 <- df3 %>% select(c(income,age,fnlwgt,capital.gain,capital.loss,hours.per.week,workclass,marital.status,relationship,race,sex,native.country,degreeobtained))
...
df5 <- df3 %>% select(c(income,age,fnlwgt,capital.gain,capital.loss,hours.per.week,workclass,relationship,race,sex,native.country,degreeobtained))
```
...
inter<-glm(income~age + fnlwgt + capital.gain + capital.loss + hours.per.week + workclass + relationship + race + sex + native.country + degreeobtained + relationship*age ,family="binomial",data=df5)
summary(inter)
VIF(inter)
